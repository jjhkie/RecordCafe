

import UIKit
import CoreLocation
import RxSwift
import RxCocoa
import SnapKit

class LocationInformationViewController: UIViewController{
    
    let disposeBag = DisposeBag()
    
    let locationManager = CLLocationManager()
    let mapView = MTMapView() // 상단의 지도 부분
    let detailList = UITableView() // 카페 목록을 보여줄 하단 tableView
    let detailListBackgroundView = DetailListBackgroundView()
    let currentLocationButton = UIButton() // 현재 위치로 이동시키는 버튼
    let VM = LocationInformationViewModel()
    
    
    override func viewDidLoad() {
        super.viewDidLoad()
        //kakao Map Setting
        mapView.delegate = self
        locationManager.delegate = self
        
        
        bind(VM)
        attribute()
        layout()
        
    }
    
    private func bind(_ VM: LocationInformationViewModel){
        
        detailListBackgroundView.bind(VM.detailListBackgroundViewModel)
        
        
        VM.setMapCenter
            .emit(to: mapView.rx.setMapCenterPoint)//하단 Reactive
            .disposed(by: disposeBag)
        VM.errorMessage
            .emit(to: self.rx.presentAlert)
            .disposed(by: disposeBag)
        
        VM.detailListCellData
            .drive(detailList.rx.items){tv, row, data in
                let cell = tv.dequeueReusableCell(withIdentifier: "DetailListCell", for: IndexPath(row: row, section: 0)) as! DetailListCell
                
                cell.setData(data)
                
                return cell
            }
            .disposed(by: disposeBag)
        
        VM.detailListCellData
            .map{
                $0.compactMap{$0.point}
            }
            .drive(self.rx.addPOIItem)
            .disposed(by: disposeBag)
        
        detailList.rx.itemSelected
            .map{$0.row}
            .bind(to: VM.detailListItemSelected)
            .disposed(by: disposeBag)
        
        VM.scrollToSelectedLocation
            .emit(to: self.rx.showSelectedLocation)
            .disposed(by: disposeBag)
        
        
        currentLocationButton.rx.tap //버튼의 tap 기능이 실행되었을 때 전달
            .bind(to: VM.currentLocationButtonTapped)
            .disposed(by: disposeBag)
    }
    
    private func attribute(){
        title = "주변 카페 찾기"
        view.backgroundColor = .white
        
        mapView.currentLocationTrackingMode = .onWithoutHeadingWithoutMapMoving
        
        currentLocationButton.setImage(UIImage(systemName: "location.fill"), for: .normal)
        currentLocationButton.backgroundColor = .white
        currentLocationButton.layer.cornerRadius = 20
        
        detailList.register(DetailListCell.self, forCellReuseIdentifier: "DetailListCell")
        detailList.separatorStyle = .none
        detailList.backgroundView = detailListBackgroundView
    }
    
    private func layout(){
        [mapView, currentLocationButton, detailList].forEach{view.addSubview($0)}
        
        mapView.snp.makeConstraints{
            $0.top.leading.trailing.equalTo(view.safeAreaLayoutGuide)
            $0.bottom.equalTo(view.snp.centerY).offset(100)
        }
        currentLocationButton.snp.makeConstraints{
            $0.bottom.equalTo(detailList.snp.top).offset(-12)
            $0.leading.equalToSuperview().offset(12)
            $0.width.height.equalTo(40)
        }
        detailList.snp.makeConstraints{
            $0.centerX.leading.trailing.equalToSuperview()
            $0.bottom.equalTo(view.safeAreaLayoutGuide).inset(8)
            $0.top.equalTo(mapView.snp.bottom)
        }
    }
}

///CLLocationManager Delegate
///해당 권한 설정
extension LocationInformationViewController: CLLocationManagerDelegate{
    func locationManager(_ manager: CLLocationManager, didChangeAuthorization status: CLAuthorizationStatus) {
        switch status{
            case .authorizedAlways,//항상
                .authorizedWhenInUse,//사용할 때
                .notDetermined:
            return
        default:
            VM.mapViewError.accept(MTMapViewError.locationAuthorizaationDenied.errorDescription)
            return
        }
    }
}

///MapView Delegate
extension LocationInformationViewController: MTMapViewDelegate{
    //현재 위치를 업데이트
    func mapView(_ mapView: MTMapView!, updateCurrentLocation location: MTMapPoint!, withAccuracy accuracy: MTMapLocationAccuracy) {
        #if DEBUG
        VM.currentLocation.accept(MTMapPoint(geoCoord: MTMapPointGeo(latitude: 37.394225, longitude: 127.110341)))
        #else
        VM.currentLocation.accept(location)
        #endif
    }
    
    //맵의 이동이 끝났을 때 센터 포인트 전달
    func mapView(_ mapView: MTMapView!, finishedMapMoveAnimation mapCenterPoint: MTMapPoint!) {
        VM.mapCenterPoint.accept(mapCenterPoint)
    }
    
    //map 의 pin을 클릭했을 때 실행되는 코드
    func mapView(_ mapView: MTMapView!, selectedPOIItem poiItem: MTMapPOIItem!) -> Bool {
        //VM.selectPOIItem.accept(poiItem)
        return false
    }
    //현재 위치를 불러오지못햇을 때 에러 return
    func mapView(_ mapView: MTMapView!, failedUpdatingCurrentLocationWithError error: Error!) {
        VM.mapViewError.accept(error.localizedDescription)
    }
}


extension Reactive where Base: MTMapView{
    var setMapCenterPoint: Binder<MTMapPoint>{
        return Binder(base) { base, point in
            base.setMapCenter(point, animated: true)
        }
    }
}

extension Reactive where Base: LocationInformationViewController{
    var presentAlert: Binder<String>{
        return Binder(base) { base, message in
            let alertController = UIAlertController(title: "문제가 발생했습니다.", message: message, preferredStyle: .alert)
            
            let action = UIAlertAction(title: "확인", style: .default, handler: nil)
            
            alertController.addAction(action)
            
            base.present(alertController, animated: true, completion: nil)
        }
    }
    
    var showSelectedLocation: Binder<Int>{
        return Binder(base) { base, row in
            let indexPath = IndexPath(row: row, section: 0)
            base.detailList.selectRow(at: indexPath, animated: true, scrollPosition: .top)
        }
    }
    
    var addPOIItem: Binder<[MTMapPoint]>{
        return Binder(base){ base, points in
            let items = points
                .enumerated()
                .map{offset, point -> MTMapPOIItem in
                    let mapPOIItem = MTMapPOIItem()
                    
                    mapPOIItem.mapPoint = point
                    mapPOIItem.markerType = .redPin
                    mapPOIItem.showAnimationType = .springFromGround
                    mapPOIItem.tag = offset
                    
                    return mapPOIItem
                }
            
            base.mapView.removeAllPolylines()
            base.mapView.addPOIItems(items)
        }
    }
    
}


